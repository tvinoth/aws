 ...
 */
S3MultiUpload.prototype.createMultipartUpload = function(type) {
    var self = this;
 ...
 */
S3MultiUpload.prototype.start = function(type) {
    this.createMultipartUpload(type);
};
...
S3MultiUpload.prototype.sendToS3 = function(data, blob, index) {
    var self = this;
    var url = data['url'];
    // if(url.indexOf('.ap-south-1') !== -1){
        // url = url.replace('.ap-south-1',"-accelerate"); // if you want all the "hello"'s in the string to be replaced    
    // }
    //oupbucket.s3-accelerate.amazonaws.com/
